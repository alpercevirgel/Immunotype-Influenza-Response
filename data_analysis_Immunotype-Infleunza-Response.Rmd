...
df_model <- df_model_3 %>% #dataset with clusters
    dplyr::mutate(`Day 0 HI titer` = dplyr::recode(T0_l3, "1"=":: <40",  "2"=":: 40-80",  "3"=":: >80",),
                ...
                CMV = dplyr::recode(CMV, "1"=":: (+)",  "2"=":: (-)")) %>%
  droplevels()
df_model$CMV <- factor(df_model$CMV, levels = c(":: (-)",":: (+)"))

# Create new variables for each cluster
for (i in 1:9) {
  df_model[[paste0('cluster_', i)]] <- ifelse(df_model$cluster_number == i, 1, 0)}
```

## run models
```{r}
for (i in 1:9){
  model_name <- paste0("model_gaml_T4_c", i)
  formula_str <- paste0("GMT_T4 ~ sex + CMV + `Day 0 HI titer` + factor(cluster_", i, ")")
  
  model <- gamlss(as.formula(formula_str), data=df_model, family=NBI())
...
mod_fit1<-summary(model_gaml_T4_c1, save=TRUE)
as.list(mod_fit1$pvalue)$Immunotype1

mod_fit2<-summary(model_gaml_T4_c2, save=TRUE)
as.list(mod_fit2$pvalue)$Immunotype2

mod_fit3<-summary(model_gaml_T4_c3, save=TRUE)
as.list(mod_fit3$pvalue)$Immunotype3

mod_fit4<-summary(model_gaml_T4_c4, save=TRUE)
as.list(mod_fit4$pvalue)$Immunotype4

mod_fit5<-summary(model_gaml_T4_c5, save=TRUE)
as.list(mod_fit5$pvalue)$Immunotype5

mod_fit6<-summary(model_gaml_T4_c6, save=TRUE)
as.list(mod_fit6$pvalue)$Immunotype6

mod_fit7<-summary(model_gaml_T4_c7, save=TRUE)
as.list(mod_fit7$pvalue)$Immunotype7

mod_fit8<-summary(model_gaml_T4_c8, save=TRUE)
as.list(mod_fit8$pvalue)$Immunotype8

mod_fit9<-summary(model_gaml_T4_c9, save=TRUE)
as.list(mod_fit9)$`Immunotype rest`



gamlss_immunotype_pval <-rbind(
  as.list(mod_fit1$pvalue)$`Immunotype1`,
  as.list(mod_fit2$pvalue)$`Immunotype2`,
  as.list(mod_fit3$pvalue)$`Immunotype3`,
  as.list(mod_fit4$pvalue)$`Immunotype4`,
  ...
  as.list(mod_fit7$pvalue)$`Immunotype7`,
  as.list(mod_fit8$pvalue)$`Immunotype8`,
  as.list(mod_fit9$pvalue)$`Immunotype rest`
  ) %>% 
  as.data.frame() %>% 
  rename("p"="V1") %>%
  adjust_pvalue(method = "BH", p.col = "p") 
gamlss_immunotype_pval
```
